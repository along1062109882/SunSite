...
class Annuals extends Common
{
    ...
    public function get_annual_data(){
        ...
        $datas = Annual::where(['deleted_at'=> null])->field('id,publish_time,state')->with('getContent')->order('publish_time','desc')->limit($offset,$limit)->select()->toArray();
        $count = Annual::where(['deleted_at'=> null])->count();
    ...
    public function annualEdit(){
        ...
        if($id){
            $llist = Annual::where(['id'=>$id])->with('getContent,getLink')->find()->toArray();
            $list=[];
            if($llist){
                $list['id'] = $llist['id'];
    ...
    public function annual_commit(){
        $data = $this->request->post();
        $id = $data['id'];
        if (isset($id) && !empty($id)) {
            //编辑
            $posts = Annual::find(['id'=>$data['id']]);
            ...
            if($posts->save()){
                $check_hk_name = Content::where(['owner_id'=>$id,'language'=>0,'owner_type'=>'annual','owner_field'=>'Name'])->find();
                $check_cn_name = Content::where(['owner_id'=>$id,'language'=>1,'owner_type'=>'annual','owner_field'=>'Name'])->find();
                $check_en_name = Content::where(['owner_id'=>$id,'language'=>2,'owner_type'=>'annual','owner_field'=>'Name'])->find();
                $check_hk_content = Content::where(['owner_id'=>$id,'language'=>0,'owner_type'=>'annual','owner_field'=>'Content'])->find();
                $check_cn_content = Content::where(['owner_id'=>$id,'language'=>1,'owner_type'=>'annual','owner_field'=>'Content'])->find();
                $check_en_content = Content::where(['owner_id'=>$id,'language'=>2,'owner_type'=>'annual','owner_field'=>'Content'])->find();
                if($check_hk_name){
                    Content::update(['data'=>$data['title_hk']],['owner_id'=>$id,'language'=>0,'owner_type'=>'annual','owner_field'=>'Name']);
                }else{
                    $content = new Content();
                    $content->data = $data['title_hk'];
                    $content->language = 0;
                    $content->owner_id = $id;
                    $content->owner_type = 'annual';
                    $content->owner_field = 'Name';
                    $content->save();
                }
                if($check_cn_name){
                    Content::update(['data'=>$data['title_cn']],['owner_id'=>$id,'language'=>1,'owner_type'=>'annual','owner_field'=>'Name']);
                }else{
                    $content = new Content();
                ...
                if($check_en_name){
                    Content::update(['data'=>$data['title_en']],['owner_id'=>$id,'language'=>2,'owner_type'=>'annual','owner_field'=>'Name']);
                }else{
                    $content = new Content();
                ...
                if($check_hk_content){
                    Content::update(['data'=>$data['content_hk']],['owner_id'=>$id,'language'=>0,'owner_type'=>'annual','owner_field'=>'Content']);
                }else{
                    $content = new Content();
                ...
                if($check_cn_content){
                    Content::update(['data'=>$data['content_cn']],['owner_id'=>$id,'language'=>1,'owner_type'=>'annual','owner_field'=>'Content']);
                }else{
                    $content = new Content();
                ...
                if($check_en_content){
                    Content::update(['data'=>$data['content_en']],['owner_id'=>$id,'language'=>2,'owner_type'=>'annual','owner_field'=>'Content']);
                }else{
                    $content = new Content();
                ...
                if($_FILES['file']['name']){
                    ...
                    $list->type = 0;
                    if($list->save()){
                        $check = LinkTarget::where(['owner_type'=>'annual','owner_id'=>$id])->find();
                        if(!$check){
                            $new = new LinkTarget();
                            $new->owner_id = $id;
                            $new->owner_type = 'annual';
                            $new->link_id = $list->id;
                            $new->owner_field = 'all';
                            $new->type = 0;
                            $new->save();
                        }else{
                            LinkTarget::update(['link_id'=>$list->id],['owner_type'=>'annual','owner_id'=>$id]);
                        }
            ...
            if($posts->save()){
                ...
                if($_FILES['file']['name']){
                    ...
                    $list->type = 0;
                    if($list->save()){
                        $check = LinkTarget::where(['owner_type'=>'annual','owner_id'=>$posts->id,'link_id'=>$list->id])->find();
                        if(!$check){
                            $new = new LinkTarget();
                            ...
                            LinkTarget::update(['link_id'=>$list->id],['owner_type'=>'annual','owner_id'=>$posts->id]);
                        }
                        return json(Result::success('成功'));
                    }
                    return json(Result::error('失敗'));
